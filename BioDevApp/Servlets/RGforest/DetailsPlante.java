...
/**
 * Servlet implementation class DetailsPlante
 */
@WebServlet("/DetailsPlante")
public class DetailsPlante extends HttpServlet {
...
HttpSession session = request.getSession();
		...
		String nesp = request.getParameter("numESPP");
		int numESP = Integer.valueOf(nesp)-1;
		session.setAttribute("numESPP", numESP);
		
		String iesp = request.getParameter("idESPP");
		int idESP = Integer.valueOf(iesp);
		session.setAttribute("idESPP", idESP);
		...
		this.getServletContext().getRequestDispatcher("/WEB-INF/JSP/RGForest/detailsPlante.jsp").forward(request, response);
